...
<div class=top><h1>LED Matrix Manager</h1><sl-badge id=status>Loading</sl-badge></div><div id=app></div><h3>Status console</h3><pre id=console class=console></pre>
...
function render(){let statusEl=document.getElementById('status'), consoleEl=document.getElementById('console'); statusEl.textContent=S.status; consoleEl.textContent=(S.messages||[]).join('\n'); consoleEl.scrollTop=consoleEl.scrollHeight; app.innerHTML=`<sl-tab-group>${S.panels.map((p,i)=>`<sl-tab slot=nav panel=p${i}>${p.name}</sl-tab><sl-tab-panel name=p${i}>${panelHtml(p,i)}</sl-tab-panel>`).join('')}</sl-tab-group><sl-button data-act=addpanel>＋ Add Panel</sl-button>`; S.panels.forEach((p,i)=>preview(i));}
function panelHtml(p,i){return `<div class=panel><div class=top><sl-button data-act=scan><sl-icon name=search></sl-icon> Scan</sl-button><sl-select value="${p.address||''}" data-act=addr data-i=${i} style="width:280px">${S.discovered.map(a=>`<sl-option value="${a}">${a}</sl-option>`).join('')}</sl-select><sl-button data-act=connect data-i=${i}>${p.connected?'Disconnect':'Connect'}</sl-button><span>${p.connected?'● Connected':'○ Disconnected'}</span><label>Brightness <input type=range min=0 max=100 value=${p.brightness} data-act=brightness data-i=${i}></label></div><canvas id=prev${i} class=preview></canvas><div class=toolbar><sl-button variant=primary data-act=play data-i=${i}><sl-icon name="${p.running?'stop-fill':'play-fill'}"></sl-icon> ${p.running?'Stop':'Play'}</sl-button><b>${p.running?'Running '+elapsed(p.started_at):'Stopped'}</b><sl-button data-act=clear data-i=${i}>Clear display</sl-button></div><div class=frames>${p.frames.map((f,j)=>frameRow(f,i,j,p.frames.length)).join('')}</div><sl-button data-act=addframe data-i=${i}>＋ Add Frame</sl-button></div>`}
...
async function save(){await api('state',{method:'POST',body:JSON.stringify(S)}); await load()} async function load(){S=await api('state'); render()} setInterval(load,5000); setInterval(()=>S&&S.panels.forEach((_,i)=>preview(i)),500);
