...
def _profile_data() -> dict:
    data={"panels":[{"name":p.name,"address":p.address,"connected":p.connected,"running":p.running,"brightness":p.brightness,"frames":[{"frame_type":f.frame_type.value,"duration":f.duration,"settings":dict(f.settings)} for f in p.frames]} for p in SERVICE.panels]}
