...
from .rendering import CLOCK_DIGIT_HEIGHT, CLOCK_DIGIT_STYLES, CLOCK_DIGIT_WIDTH, CLOCK_SEPARATOR_WIDTH, DEFAULT_CLOCK_DIGIT_STYLE, FONT_ALIASES, render_frame
    ...
    def do_POST(self):
        ...
        try:
            ...
            if u.path=="/api/digit_pixels":
                data=self._body(); name=Path(data.get('name') or 'digit-0.png').name; override=data.get('path'); style=data.get('style') or DEFAULT_CLOCK_DIGIT_STYLE
                path=Path(override) if override else Path(__file__).parent / 'digits' / (style if style in CLOCK_DIGIT_STYLES else DEFAULT_CLOCK_DIGIT_STYLE) / name
                if not path.exists() and not override: path=Path(__file__).parent / 'digits' / DEFAULT_CLOCK_DIGIT_STYLE / name
                ...
                if src.size != target_size: src = src.resize(target_size, Image.Resampling.NEAREST)
                pixels=[]
