...
class LedPanelService:
    def __init__(self):
        ...
        self.discovery_complete = False; self.status = "Starting"; self.tick = 0; self.last_images: dict[int, Image.Image] = {}
    ...
    def discovery_worker(self):
        while not self._stop.is_set():
            try:
                ...
                with self.lock:
                    self.discovered_addresses = sorted(set(found)); self.discovery_complete = True; self.status = f"Discovered {len(found)} panel(s)"
            except Exception as exc:
                logger.exception("Panel discovery failed"); self.status = f"Discovery failed: {exc}"
            self._stop.wait(60)
    ...
    def _connect_worker(self, idx: int, autostart: bool):
        p=self.panels[idx]
        if not p.address: return
        try:
            conn=PanelConnection(p.address); conn.connect().result(timeout=30); conn.set_brightness(p.brightness).result(timeout=10)
            with self.lock: p.connection=conn; p.connected=True; self.status=f"Connected to {p.address}"
            if autostart or p.running: self.play(idx)
        except Exception as exc:
            logger.exception("Connect failed"); self.status=f"Connect failed: {exc}"
    def disconnect(self, idx:int):
        ...
        p.connected=False; p.connection=None; self.save_state()
    def play(self, idx:int):
        p=self.panels[idx]
        if p.running: return
        p.running=True; p.started_at=time.time(); threading.Thread(target=self._run_panel, args=(idx,), daemon=True).start(); self.save_state()
    def stop(self, idx:int):
        p=self.panels[idx]; p.running=False; p.started_at=None; self.save_state()
                ...
                while p.running and time.monotonic()<end:
                    ...
                    if conn and now-last_send>=1:
                        try: conn.send_image(img).result(timeout=20); last_send=now
                            ...
                            logger.exception("Send failed"); p.connected=False; p.connection=None; self.status=f"Send failed; reconnecting: {exc}"; self._connect_worker(idx, False); last_send=now
                    self.tick+=1; time.sleep(.25)
    def state(self):
        with self.lock: return {"status": self.status, "discovered": self.discovered_addresses, "fonts": list(FONT_ALIASES), "width": PANEL_WIDTH, "height": PANEL_HEIGHT, "frame_types": [t.value for t in FrameType], "panels": [_jsonable_panel(p) for p in self.panels]}
...
<style>@font-face{font-family:LedFont;src:url('/fonts/Valencia%2012.ttf')}body{font-family:Inter,Arial,sans-serif;margin:24px;background:#f6f7fb;color:#18202a}.top{display:flex;gap:12px;align-items:center}.panel{background:white;border:1px solid #d7dce4;border-radius:14px;padding:18px;margin-top:16px;box-shadow:0 8px 26px #1d27330d}.preview,.editor{display:grid;grid-template-columns:repeat(96,10px);gap:2px;background:#111;padding:10px;border-radius:12px;width:max-content}.led{width:10px;height:10px;border-radius:50%;background:#272b31}.frames{display:grid;gap:10px}.frame{display:flex;gap:8px;align-items:center;border:1px solid #e1e5eb;padding:10px;border-radius:10px}.modal{position:fixed;inset:0;background:#0008;display:none;align-items:center;justify-content:center;z-index:20}.modal.open{display:flex}.dialog{background:white;border-radius:16px;padding:20px;max-height:90vh;overflow:auto;min-width:760px}.form{display:grid;grid-template-columns:170px 1fr;gap:10px;align-items:center}.thumbs{display:flex;gap:8px;flex-wrap:wrap}.thumb{border:1px solid #cfd6df;border-radius:8px;padding:6px;cursor:pointer}.toolbar{display:flex;gap:10px;align-items:center;margin:10px 0}.muted{color:#64748b}</style></head><body>
<div class=top><h1>LED Matrix Manager</h1><sl-badge id=status>Loading</sl-badge></div><div id=app></div>
<div id=modal class=modal><div class=dialog><div id=modalBody></div></div></div>
<script>
let S, edit={panel:0, frame:0}, drawPixels=[]; const W=96,H=16, off='#272b31';
const api=(p,o={})=>fetch('/api/'+p,{headers:{'content-type':'application/json'},...o}).then(r=>r.json());
function ledGrid(img){let html='<div class=preview>';let pix=img?.pixels||[];for(let y=0;y<H;y++)for(let x=0;x<W;x++){let c=pix[y]?.[x]||[0,0,0];let on=c.some(v=>v);html+=`<span class=led style="background:${on?`rgb(${c})`:off}"></span>`}return html+'</div>'}
function emptyPix(w=W,h=H,bg=[0,0,0]){return Array.from({length:h},()=>Array.from({length:w},()=>bg.slice()))}
function render(){document.getElementById('status').textContent=S.status; app.innerHTML=S.panels.map((p,i)=>`<div class=panel><div class=top><h2>${p.name}</h2><sl-select value="${p.address||''}" data-act=addr data-i=${i} style="width:250px">${S.discovered.map(a=>`<sl-option value="${a}">${a}</sl-option>`).join('')}</sl-select><sl-button data-act=connect data-i=${i}>${p.connected?'Disconnect':'Connect'}</sl-button><span>${p.connected?'● Connected':'○ Disconnected'}</span><label>Brightness <input type=range min=0 max=100 value=${p.brightness} data-act=brightness data-i=${i}></label></div><div id=prev${i}></div><div class=toolbar><sl-button variant=primary data-act=play data-i=${i}><sl-icon name="${p.running?'stop-fill':'play-fill'}"></sl-icon> ${p.running?'Stop':'Play'}</sl-button><b>${p.running?'Running '+elapsed(p.started_at):'Stopped'}</b><sl-button data-act=clear data-i=${i}>Clear display</sl-button></div><div class=frames>${p.frames.map((f,j)=>frameRow(f,i,j,p.frames.length)).join('')}</div><sl-button data-act=addframe data-i=${i}>＋ Add Frame</sl-button></div>`).join('')+`<sl-button data-act=addpanel>＋ Add Panel</sl-button>`; S.panels.forEach((p,i)=>preview(i));}
...
async function preview(i){let r=await api(`preview?panel=${i}`);document.getElementById('prev'+i).innerHTML=ledGrid(r)}
async function save(){await api('state',{method:'POST',body:JSON.stringify(S)}); await load()}
async function load(){S=await api('state'); render()} setInterval(load,5000);
...
document.addEventListener('click',async e=>{let b=e.target.closest('[data-act]');if(!b)return;let {act,i,j}=b.dataset;if(act=='connect')await api(`panel/${i}/${S.panels[i].connected?'disconnect':'connect'}`,{method:'POST'}); if(act=='play')await api(`panel/${i}/${S.panels[i].running?'stop':'play'}`,{method:'POST'}); if(act=='clear')await api(`panel/${i}/clear`,{method:'POST'}); if(act=='addpanel')S.panels.push({name:'Panel '+(S.panels.length+1),frames:[{frame_type:'Text',duration:10,settings:{}}],brightness:80}); if(act=='addframe')S.panels[i].frames.push({frame_type:'Text',duration:10,settings:{}}); if(act=='remove')S.panels[i].frames.splice(j,1); if(act=='up'){let a=S.panels[i].frames;[a[j],a[j-1]]=[a[j-1],a[j]]} if(act=='down'){let a=S.panels[i].frames;[a[j],a[+j+1]]=[a[+j+1],a[j]]} if(['addpanel','addframe','remove','up','down'].includes(act))await save(); if(act=='settings')openSettings(+i,+j); await load()});
function input(k,v,type='text'){return `<input data-k="${k}" type=${type} value="${v??''}">`}
async function openSettings(i,j){edit={panel:i,frame:j};let f=S.panels[i].frames[j],s=f.settings||{},body=`<h2>${f.frame_type} settings</h2><div id=dialogPrev></div><div class=form>`; if(f.frame_type=='Text')body+=`<label>Message</label><textarea data-k=message>${s.message||'Hello'}</textarea>`; if(f.frame_type=='Date')body+=`<label>Date format</label>${input('date_format',s.date_format||'%d/%m/%Y')}`; if(f.frame_type=='Image')body+=`<label>Image file</label><input data-k=path value="${s.path||''}"><label></label><sl-button data-act=draw><sl-icon name=palette></sl-icon> Draw</sl-button><label>Image display</label><select data-k=display><option>Resize to fit</option><option>Stretch to fit</option><option>Crop to fit</option></select>`; if(['Text','Date','Clock'].includes(f.frame_type))body+=`<label>Icon path</label><span><input data-k=icon_path value="${s.icon_path||''}"><sl-button data-act=clearicon>Clear image</sl-button></span><label>Foreground</label>${input('foreground',rgbhex(s.foreground||[255,255,0]),'color')}<label>Background</label>${input('background',rgbhex(s.background||[0,0,0]),'color')}`; if(['Text','Date'].includes(f.frame_type))body+=`<label>Font</label><select data-k=font>${S.fonts.map(x=>`<option ${x==(s.font||'VCR OSD Mono')?'selected':''}>${x}</option>`)}</select><label>Font size</label>${input('font_size',s.font_size||16,'number')}<label>Horizontal spacing</label>${input('horizontal_spacing',s.horizontal_spacing||0,'number')}<label>Vertical offset</label>${input('vertical_offset',s.vertical_offset||0,'number')}`; if(f.frame_type=='Text')body+=`<label>Scrolling</label><select data-k=scrolling><option>None (Wrap)</option><option>Right to left</option><option>Left to right</option><option>Top to bottom</option><option>Bottom to top</option></select><label>Scroll speed</label>${input('scroll_speed',s.scroll_speed||4,'number')}`; if(f.frame_type=='Clock')body+=`<label>12/24 hour time</label><select data-k=time_mode><option>24-hour</option><option>12-hour</option></select><label>Show seconds</label><input data-k=show_seconds type=checkbox ${s.show_seconds?'checked':''}><label>Flash separator</label><input data-k=flash_separator type=checkbox ${s.flash_separator?'checked':''}><label>Digit spacing</label><input data-k=digit_spacing type=range min=0 max=10 value="${s.digit_spacing??2}"><label></label><sl-button data-act=digits>Edit digits</sl-button>`; body+=`</div><div class=toolbar><sl-button variant=primary data-act=apply>Apply</sl-button><sl-button data-act=close>Cancel</sl-button></div>`; modalBody.innerHTML=body; modal.classList.add('open'); dialogPreview();}
...
async function dialogPreview(){collect();let r=await api(`preview_frame`,{method:'POST',body:JSON.stringify(S.panels[edit.panel].frames[edit.frame])}); dialogPrev.innerHTML=ledGrid(r)}
modal.addEventListener('input',e=>{if(e.target.dataset.k)dialogPreview()}); modal.addEventListener('click',async e=>{let a=e.target.closest('[data-act]')?.dataset.act;if(a=='close')modal.classList.remove('open');if(a=='clearicon'){modalBody.querySelector('[data-k=icon_path]').value='';dialogPreview()}if(a=='apply'){collect();modal.classList.remove('open');await save()}if(a=='draw')openDraw(false);if(a=='digits')openDraw(true)});
function openDraw(digits){let w=digits?8:W,h=digits?16:H;drawPixels=emptyPix(w,h);modalBody.innerHTML=`<h2>${digits?'Edit clock digits':'Draw image'}</h2>${digits?'<div class=thumbs>'+['digit-0.png','digit-1.png','digit-2.png','digit-3.png','digit-4.png','digit-5.png','digit-6.png','digit-7.png','digit-8.png','digit-9.png','separator.png','am.png','pm.png'].map(x=>`<button class=thumb data-name=${x}>${x}</button>`).join('')+'</div>':''}<div class=toolbar><label>Background <input id=bg type=color value=#000000></label><label>Foreground <input id=fg type=color value=#ff0></label><sl-button id=clr>Clear</sl-button></div><div id=ed class=editor style="grid-template-columns:repeat(${w},16px)"></div><div class=toolbar><sl-button id=keep variant=primary>${digits?'Save':'Keep'}</sl-button><sl-button id=discard>Discard</sl-button></div>`;let cur='digit-0.png';function paint(){ed.innerHTML='';for(let y=0;y<h;y++)for(let x=0;x<w;x++)ed.innerHTML+=`<span class=led data-x=${x} data-y=${y} style="width:16px;height:16px;background:${rgbhex(drawPixels[y][x])}"></span>`}paint(); ed.onclick=e=>{let l=e.target.closest('.led');if(l){drawPixels[l.dataset.y][l.dataset.x]=hexrgb(fg.value);paint()}}; clr.onclick=()=>{drawPixels=emptyPix(w,h,hexrgb(bg.value));paint()}; discard.onclick=()=>modal.classList.remove('open'); keep.onclick=async()=>{let res=await api(digits?'save_digit':'save_drawing',{method:'POST',body:JSON.stringify({pixels:drawPixels,name:cur})}); if(digits){let f=S.panels[edit.panel].frames[edit.frame];f.settings.digit_overrides={...(f.settings.digit_overrides||{}),[cur]:res.path}} else S.panels[edit.panel].frames[edit.frame].settings.path=res.path; modal.classList.remove('open'); await save()}; modalBody.querySelectorAll('.thumb').forEach(t=>t.onclick=()=>{cur=t.dataset.name;drawPixels=emptyPix(w,h);paint()});}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path=="/api/preview":
            idx=int(parse_qs(u.query).get('panel',[0])[0]); img=SERVICE.last_images.get(idx) or render_frame(SERVICE.panels[idx].frames[0], SERVICE.tick); return self._json({"pixels": list(map(list, img.getdata())) and [[list(img.getpixel((x,y))) for x in range(PANEL_WIDTH)] for y in range(PANEL_HEIGHT)]})
        self._send(404)
    def do_POST(self):
        u=urlparse(self.path); parts=u.path.strip('/').split('/')
        try:
            if u.path=="/api/state":
                raw=self._body(); SERVICE.panels=[]
                for pd in raw.get('panels',[]): SERVICE.panels.append(PanelState(pd.get('name','Panel'), pd.get('address'), False, [FrameConfig(FrameType(fd.get('frame_type','Text')), fd.get('duration',10), fd.get('settings',{})) for fd in pd.get('frames',[])], pd.get('running',False), pd.get('brightness',80), pd.get('started_at')))
                SERVICE.save_state(); return self._json({"ok":True})
            if u.path=="/api/preview_frame":
                fd=self._body(); img=render_frame(FrameConfig(FrameType(fd.get('frame_type','Text')), fd.get('duration',10), fd.get('settings',{})), SERVICE.tick); return self._json({"pixels":[[list(img.getpixel((x,y))) for x in range(PANEL_WIDTH)] for y in range(PANEL_HEIGHT)]})
