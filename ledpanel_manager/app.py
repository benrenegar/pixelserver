...
function updateStatus(){let statusEl=document.getElementById('status'), consoleEl=document.getElementById('console'); statusEl.textContent=S.status; consoleEl.textContent=(S.messages||[]).join('\n'); consoleEl.scrollTop=consoleEl.scrollHeight}function render(){updateStatus(); app.innerHTML=`<sl-tab-group>${S.panels.map((p,i)=>`<sl-tab slot=nav panel=p${i}>${p.name}</sl-tab><sl-tab-panel name=p${i}>${panelHtml(p,i)}</sl-tab-panel>`).join('')}</sl-tab-group><sl-button data-act=addpanel>＋ Add Panel</sl-button>`; S.panels.forEach((p,i)=>preview(i));}
