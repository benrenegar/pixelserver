...
function drawCanvas(canvas,pixels,scale=10){let targetW=W*scale,targetH=H*scale;if(canvas.width!==targetW)canvas.width=targetW;if(canvas.height!==targetH)canvas.height=targetH;canvas.style.width='100%';canvas.style.height='auto';let ctx=canvas.getContext('2d');ctx.fillStyle='#111';ctx.fillRect(0,0,targetW,targetH);for(let y=0;y<H;y++)for(let x=0;x<W;x++){let c=pixels?.[y]?.[x]||[0,0,0],on=c.some(v=>v),cc=on?c:off;ctx.fillStyle=`rgb(${cc[0]},${cc[1]},${cc[2]})`;ctx.beginPath();ctx.arc(x*scale+scale/2,y*scale+scale/2,scale*.32,0,Math.PI*2);ctx.fill()}}
