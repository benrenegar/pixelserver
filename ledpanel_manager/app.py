...
const DEF_FG='#00ffb3', DEF_FONT='Avante 8', DEF_SIZE=8, PALETTE=['#000000','#808080','#ff0000','#ffff00','#00ff00','#00ffff','#0000ff','#ff00ff','#ffffff','#00ffb3','#7fbfff','#7f7fff','#ff007f','#7f7f00','#7f4000'];
...
async function openIconEditor(){let f=S.panels[edit.panel].frames[edit.frame];modalBody.innerHTML=editorHtml('Edit/upload icon',16,16)+`<div class=toolbar><button id=keep variant=primary>Keep</button><button id=discard>Close</button></div>`;await setupPixelEditor('icon_path',16,16,f.settings.icon_path);document.getElementById('discard').onclick=()=>openSettings(edit.panel,edit.frame);document.getElementById('keep').onclick=async()=>{let res=await api('save_drawing',{method:'POST',body:JSON.stringify({pixels:drawPixels})});f.settings.icon_path=res.path;await save();openSettings(edit.panel,edit.frame)}}
        ...
        try:
            ...
            if u.path=="/api/upload_image":
                ...
                if data.get('width') and data.get('height'):
                    src=Image.open(path).convert('RGB').resize((int(data['width']), int(data['height'])), Image.Resampling.LANCZOS)
                    src.save(path)
            ...
            if u.path=="/api/image_pixels":
                ...
                src=Image.open(path).convert('RGB').resize((w, h), Image.Resampling.LANCZOS)
